    ...
    if (!Array.isArray(elections)) {
      container.innerHTML = `
        <div class="empty-state">
          <p>Unable to load elections at the moment.</p>
        </div>
      `;
      return;
    }
...
function renderElections(elections) {
  ...
  elections.forEach((election) => {
    ...
    const active = election.active && now <= endTime;

    card.innerHTML = `
      <div class="election-top">
        <h2>${election.title}</h2>
        <span class="election-status ${active ? "active" : "closed"}">
          ${active ? "Active" : "Closed"}
        </span>
      </div>
      <p class="group-text">Group: ${election.group}</p>
      <p class="timer">Start: ${new Date(election.startTime).toLocaleString()}</p>
      <p class="timer">Ends: ${endTime.toLocaleString()}</p>
